...
class MainCoordinator:
    def __init__(self):
        ...
        limo_state_matrix_pub = rospy.Publisher('/limo_state_matrix', limo_state_matrix, queue_size=10)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            for i in range(len(order_list)):
                limo_state = calc_qp_info(search_info, i)
                limo_state_mat.limos.append(limo_state)

            rospy.loginfo(f"Publishing LimoStateMatrix: {limo_state_mat}")
            limo_state_matrix_pub.publish(limo_state_mat)
            self.rate.sleep()
