...
class QPSolverCAV2:
    def __init__(self, cav2_id, cav1_id):
        ...
        self.deltaSafetyDistance = 0.5# Minimum safety distance (meters)
    ...
    def limo_state_callback(self, data):
        ...
        for limo in data.limos:
            if limo.limoID == self.cav2_id:
                self.state = [limo.d0, limo.vel, limo.d2]
            elif limo.limoID == self.cav1_id:
                cav1_data = [limo.limoID, limo.d0, limo.v2, limo.d1]
    ...
    def OCBF_SecondOrderDynamics(self, matrix_const, state, vd):
        ...
        if matrix_const[2][0] != -1:
            ...
            v0 = matrix_const[2][2]  # Velocity of the conflicting vehicle (CAV1)
    ...
    def recalc_qp(self):
        if self.state is not None and self.matrix_const is not None:
            vd = 1 # Reference velocity is the current velocity for simplicity
