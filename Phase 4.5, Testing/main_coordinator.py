...
class MainCoordinator:
    def __init__(self):
        ...
        # Initialize all CAVs
        self.cavs = {ID: CAV(ID, True, enter, exit) for ID, (enter, exit) in cav_configs.items()}
    ...
    def insert_cav_in_link_zone(self, new_cav):
        ...
        for cav in conflicting_cavs:
            ...
            print("conflicting cavs", cav.ID, manhattan_distance)
        ...
        if len(self.order_list["Link Zone"]) == 0:
            self.order_list["Link Zone"].append(new_cav)
        elif temp == len(conflicting_cavs) - 1:
            ...
            print("next conflicting cav",next_conflicting_cav)
            for index, cav in enumerate(self.order_list["Link Zone"]): #conditions are right, but link zone is empty
                print(new_cav.ID, index, cav.ID, next_conflicting_cav.ID)
    ...
    def check_zone_transition(self):
        ...
        for cav_id, cav in self.cavs.items():
            ...
            if "Link Zone" in cav.zone and "Link Zone" not in previous_zones:
                self.insert_cav_in_link_zone(cav)
                print("inserting cav", cav.ID, "into link zone")
