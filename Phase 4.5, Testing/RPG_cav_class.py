...
class CAV():
    ...
    def generate_map(self, starting_pt):
        ...
        self.m_to_c = {
            ...
            "ranges": [self.act_range_m, self.act_range_l, self.act_range_h, self.act_range_c],
            "circles": [self.circle_m, self.circle_l, self.circle_h, self.circle_c],
            "PIDs": [self.path_G_PID, self.path_J_PID, self.path_F_PID],
            "curve_PIDs": [self.circle_m_PID, self.circle_l_PID, self.circle_h_PID, self.circle_c_PID]
        }
    ...
    def regenerate_map(self, starting_pt=0, first_time=False):
        ...
        if first_time == True:
            ...
            if starting_pt == self.pt_f:
                self.enter_path = random.choice(self.f_paths)
            elif starting_pt == self.pt_e:
                self.enter_path = random.choice(self.e_paths)
            elif starting_pt == self.pt_m:
                self.enter_path = random.choice(self.m_paths)
            elif starting_pt == self.pt_t:
                self.enter_path = random.choice(self.t_paths)
            elif starting_pt == self.pt_n:
                self.enter_path = random.choice(self.n_paths)
            ...
            if self.turning_pts[-1] == self.pt_c:
                self.exit_path = random.choice(self.c_paths)
            elif self.turning_pts[-1] == self.pt_j:
                self.exit_path = random.choice(self.j_paths)
            elif self.turning_pts[-1] == self.pt_s:
                self.exit_path = random.choice(self.s_paths)
            elif self.turning_pts[-1] == self.pt_q:
                self.exit_path = random.choice(self.q_paths)
    ...
    def regenerate_entrace_to_link(self):
        #appaend path to enter link
