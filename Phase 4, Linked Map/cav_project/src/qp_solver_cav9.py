...
class QPSolverCAV9:
    def __init__(self, cav9_id, cav8_id, cav7_id, cav6_id, cav5_id, cav4_id, cav3_id, cav2_id, cav1_id, cav10_id, cav11_id):
        self.cav9_id = cav9_id  # CAV9 with the solver (first lateral)
        self.cav8_id = cav8_id  # CAV8 preceding CAV9 (rear-end constraint)
        self.cav7_id = cav7_id  # Receding
        self.cav6_id = cav6_id  # Receding
        self.cav5_id = cav5_id  # Receding
        self.cav4_id = cav4_id  # Receding
        self.cav3_id = cav3_id  # Receding
        self.cav2_id = cav2_id  # Receding
        self.cav1_id = cav1_id  # Receding
        self.cav10_id = cav10_id
        self.cav11_id = cav11_id
        self.u_min = -10  # Minimum control input (deceleration)
        self.u_max = 1  # Maximum control input (acceleration)
        self.phiRearEnd = 1  # Reaction time for rear-end safety constraint
        self.phiLateral = 0.1  # Reaction time for lateral safety constraint
        self.deltaSafetyDistance = 1  # Minimum safety distance (meters)
        self.v_min = 0  # Minimum velocity
        self.v_max = 1  # Maximum velocity

        rospy.init_node("qp_solver_" + self.cav9_id)
        self.qp_solution_pub = rospy.Publisher('/qp_solution_' + self.cav9_id, QP_solution, queue_size=10)
    ...
    def recalc_qp(self):
        if self.state is not None:
            ...
            qp_solution_msg.u = u
            print("Computed control input u qp 9:", u)
...
if __name__ == '__main__':
    solver = QPSolverCAV9("limo793", "limo777", "limo813", "limo780", "limo799", "limo789", "limo795", "limo155", "limo770", "limo808", "limo787")
    solver.run()
